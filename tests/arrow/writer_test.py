...
from resoto_datalink.arrow.writer import (
...
def test_normalize() -> None:
    ...
    normalize(
        NormalizationPath(path=["baz"], convert_to=ParquetMap(convert_values_to_str=True)),
        object,
    )
    ...
    normalize(
        NormalizationPath(path=["bar", None], convert_to=ParquetMap(convert_values_to_str=True)),
        object,
    )
